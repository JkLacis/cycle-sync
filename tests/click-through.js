...
function harness() {
  ...
  // Coach tests use the offline (scripted) coach: deterministic and never spends API credits.
  coachMode = "offline";
  ...
  t("step12", "demo date panel", () => { ok("step12", "demo bar shown with date", !q("#demo-bar").hidden && q("#demo-bar").textContent.includes("Change date")); click("#demo-bar"); const btns = qa("#info-body .demo-phase"); const s = store.settings.load(); ok("step12", "4 phase buttons, each date lands in its phase", btns.length === 4 && btns.every((b, i) => getPhaseForDate(parseLocalDate(b.dataset.demoDate), s) === CYCLE_ORDER[i]), btns.map((b) => b.dataset.demoDate).join(",")); ok("step12", "date picker", q("#demo-date-input") !== null); });
  ...
  const HANDLED = "[data-screen], [data-go], [data-back], [data-sheet], [data-edit], [data-day], [data-add-on], #ring, [data-date], [data-event], [data-delete], [data-phase], [data-month], [data-log], [data-ask], [data-retry], [data-session], [data-soon], [data-integration], [data-confirm-reset], [data-close-sheet], [data-demo-date], #demo-bar, #clear-chat-btn, #add-task-btn, #plan-today-btn, #see-all-btn, #coach-history-btn, #int-see-all-btn, #bell-btn, #report-btn, #export-btn, #reset-btn";
